...
public class Filter implements java.io.Serializable {
    ...
    private static final Map<FilterOp, Integer> _typeMap = new HashMap<FilterOp, Integer>();
    ...
    public Filter(String name, String ruleString, RuleTerm r, int h) {
        _name = name;
        _rule = r;
        _ruleString = ruleString;
        _minMatchScore = (h < 1) ? 1 : h;
    }

    public Filter(String name, RuleTerm r) {
        _name = name;
        _rule = r;
        _ruleString = r.toString();
        _minMatchScore = _defaultMinMatchScore;
    }

    public Filter(String name, RuleTerm r, int h) {
        _name = name;
        _rule = r;
        _ruleString = r.toString();
        _minMatchScore = (h < 1) ? 1 : h;
    ...
    public static FilterOp getTermType(int n) {
        return _termTypes[n];
    }

    public static int getValue(FilterOp op) {
        return _typeMap.get(op);
    }
    ...
    public int getMatchScore(NewsItem article, int numTokens, Map<String, Score> matchScores) {
        try {
    ...
    public int getMatchScore(NewsItem article, Map<Concept, Score> matchedConcepts, Map<Category, Score> matchedCats) {
        try {
            return _rule.getMatchScore(this, article, matchedConcepts, matchedCats);
        } catch (Exception e) {
            _log.error("Caught exception in match score for filter: " + _key + ": " + _name + ": " + _ruleString, e);
            return 0;
        }
    ...
    public void collectUsedConcepts(Set<Concept> concepts) {
        _rule.collectUsedConcepts(concepts);
    }
...
// ############### NESTED CLASSES ############
    ...
    public static abstract class RuleTerm implements java.io.Serializable {
        ...
        abstract public int getMatchScore(Filter f, NewsItem article, int numTokens, Map<String, Score> matchScores);

        abstract public int getMatchScore(Filter f, NewsItem article, Map<Concept, Score> matchedConcepts, Map<Category, Score> matchedCats);
    }
    ...
    public static class LeafConcept extends RuleTerm {
        ...
        public LeafConcept(Concept c) {
            _concept = c;
            _minConceptHitScore = Filter._defaultMinConceptHitScore;
        }

        public LeafConcept(Concept c, Integer n) {
            _concept = c;
            _minConceptHitScore = (n == null) ? Filter._defaultMinConceptHitScore : ((n < 1) ? 1 : n);
        }

        public FilterOp getType() {
            return FilterOp.LEAF_CONCEPT;
        }
        ...
        public int getMinOccurences() {
            return _minConceptHitScore;
        }

        public String toString() {
            return _concept.getName() + (_minConceptHitScore == 1 ? "" : ":" + _minConceptHitScore);
        }

        public void print(PrintWriter pw) {
            pw.println(_indent + _concept.getLexerToken().getToken() + (_minConceptHitScore == 1 ? "" : ":" + _minConceptHitScore));
        }
    ...
    public static class LeafFilter extends RuleTerm {
        ...
        public LeafFilter(final Filter f) {
            _filt = f;
        }

        public FilterOp getType() {
            return FilterOp.LEAF_FILTER;
        }
        ...
        public String toString() {
            return "[" + _filt.getName() + "]";
        }

        public void print(PrintWriter pw) {
            pw.println(_indent + _filt.getName());
        }

        public void collectUsedConcepts(final Set<Concept> usedConcepts) {
            _filt.collectUsedConcepts(usedConcepts);
        }

        public int getMatchScore(Filter f, NewsItem article, int numTokens, Map<String, Score> matchScores) {
            return _filt.getMatchScore(article, numTokens, matchScores);
        }

        public int getMatchScore(Filter f, NewsItem article, Map<Concept, Score> matchedConcepts, Map<Category, Score> matchedCats) {
            return _filt.getMatchScore(article, matchedConcepts, matchedCats);
        }
    }
    ...
    public static class SourceFilter extends RuleTerm {
        ...
        public SourceFilter(final NR_SourceCollection coll) {
            _srcColl = coll;
        }

        public FilterOp getType() {
            return FilterOp.SOURCE_FILTER;
        }
        ...
        public String toString() {
            return "[" + _srcColl.getName() + "]";
        }

        public void print(PrintWriter pw) {
            pw.println(_indent + _srcColl.getName());
        }
    ...
    public static class LeafCategory extends RuleTerm {
        ...
        public LeafCategory(final Category c) {
            _cat = c;
        }

        public FilterOp getType() {
            return FilterOp.LEAF_CAT;
        }
        ...
        public void print(PrintWriter pw) {
            pw.println(_indent + _cat.getName());
        }
        ...
        public int getMatchScore(Filter f, NewsItem article, Map<Concept, Score> matchedConcepts, Map<Category, Score> matchedCats) {
            ...
            if (mc == null) {
                if (_log.isDebugEnabled())
                    _log.debug("CAT " + _cat.getName() + " in issue " + _cat.getIssue().getName() + " being processed recursively!");
                mc = _cat.getMatchScore(article, matchedConcepts, matchedCats);
            }
            return mc.value();
        }
    }
    ...
    public static class ContextTerm extends RuleTerm {
        ...
        public int getMatchScore(Filter f, NewsItem article, Map<Concept, Score> matchedConcepts, Map<Category, Score> matchedCats) {
            ...
            return _r.getMatchScore(f, article, matchedConcepts, matchedCats);
        }
    }
    ...
    public static class NegTerm extends RuleTerm {
        ...
        public NegTerm(final RuleTerm t) {
            _t = t;
        }

        public FilterOp getType() {
            return FilterOp.NOT_TERM;
        }
        ...
        public int getMatchScore(Filter f, NewsItem article, Map<Concept, Score> matchedConcepts, Map<Category, Score> matchedCats) {
            // FIXME: This is very strict!
            final int score = (1 - _t.getMatchScore(f, article, matchedConcepts, matchedCats));
            return (score > 0) ? f._minMatchScore : 0;
        }
    }
    ...
    public static class AndOrTerm extends RuleTerm {
        ...
        public AndOrTerm(final FilterOp op, final RuleTerm lt, final RuleTerm rt) {
            _op = op;
            _lTerm = lt;
            _rTerm = rt;
        }
        ...
        public int getMatchScore(Filter f, NewsItem article, Map<Concept, Score> matchedConcepts, Map<Category, Score> matchedCats) {
            final int lscore = _lTerm.getMatchScore(f, article, matchedConcepts, matchedCats);
            final int rscore = _rTerm.getMatchScore(f, article, matchedConcepts, matchedCats);
            if (_op == FilterOp.AND_TERM) {
                if ((lscore >= f._minMatchScore) && (rscore >= f._minMatchScore))
                    return (lscore + rscore) / 2;
                else
    ...
    public static class ProximityTerm extends RuleTerm {
        ...
        public FilterOp getType() {
            return FilterOp.PROXIMITY_TERM;
        }
        ...
        public int getProximityVal() {
            return _proximityVal;
        }

        public String toString() {
            return _c1.getName() + " ~" + _proximityVal + " " + _c2.getName();
        }

        public void collectUsedConcepts(final Set<Concept> usedConcepts) {
            usedConcepts.add(_c1);
            usedConcepts.add(_c2);
        }
        ...
        private int numProximityMatches(Score s1, Score s2) {
