...
public final class IOUtils
{
   private static Log _log = LogFactory.getLog(IOUtils.class);
     ...
     */
	public static void copyInputToOutput(InputStream in, OutputStream out) throws java.io.IOException
	{
	...
	{
		copyInputToOutput(input, new FileOutputStream(localCopyName));
	}
	...
	private static InputStream getInputStream(URL u) throws java.io.IOException
	{
		...
		connection.addRequestProperty("User-Agent", "NewsRack");
	...
	public static InputStream getURLInputStream(URL u, int numRetries) throws IOException
	{
		...
		while (true) {
			try {
				INFO("getting input stream for " + u + "; Try # " + numTries);
				return getInputStream(u);
			}
				/* Catch timeouts and other exceptions and keep trying */
