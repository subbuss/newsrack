...
$prefix      = "ko";
$defSiteRoot = "http://www.kanglaonline.com";
$url         = "$defSiteRoot/index.php";
$artnum1     = &OpenArtNumFile("30000");
##print "GOT artnum1 - $artnum1\n";
...
##
      ## The next line uses information about Kangla Online's site structure
		## http://www.kanglaonline.com/index.php?template=headline&newsid=35288&typeid=1
   if ($url =~ m{\&newsid=(\d+).*\&typeid=(\d+)}) {
		$artNum = $1;
		$type = $2;
		print "Article number = $artNum; type - $type\n";
		next if (($type > 0) && ($artNum < $startingArtNum));
