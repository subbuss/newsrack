...
/** Note that this class extends Category and inherits many fields and methods **/
    ...
    * as a string representation
	...
	public String getLastUpdateTime_String()
	{
		if (_lastUpdateTime == null)
			...
			synchronized (GlobalConstants.DF) {
				return (GlobalConstants.DF.format(_lastUpdateTime));
			}
	}
	...
	private int readInExistingCatMap()
	{
		...
		String catMapFile = dir + File.separator + osFriendlyName + "_ISSUE_CAT_MAP.xml";
	...
	{
		return (_db == null) ? "" : _db.getUserSpaceWorkDir(_user);
	}
...
//			JFlex.Main.main(args);
		...
		// FIXME: Does this assume that the underlying system is a Unix system??
			...
			String args[];
			String webappPath = GlobalConstants.getWebappPath();
			if (webappPath != null) {
					// Servlet based execution
				args = new String[4];
				args[0] = GlobalConstants.getProperty("java.compiler");
				args[1] = "-classpath";
	...
	public void classifyArticle(NewsItem ni, int numTokens, Hashtable tokTable)
	{
		...
		for (Category c: getCategories()) {
			Count mv = c.getMatchCount(ni, numTokens, tokTable);
	...
	public String getRSSDir() { return GlobalConstants.getBaseRssDir() + File.separator + _taxonomyPath; }

	public String getRSSFeedURL() {
		return GlobalConstants.getServerURL() + File.separator
						+ GlobalConstants.getProperty("rssDir") + File.separator
		            + _taxonomyPath + File.separator + GlobalConstants.getProperty("rssfeedName");
	}
	...
	private void initFeed()
	{
		...
		String feedName  = getRSSDir() + File.separator + GlobalConstants.getProperty("rssfeedName");
		String feedTitle = "RSS feed for topic " + _taxonomyPath;
