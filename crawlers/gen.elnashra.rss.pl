...
sub ProcessPage
{
   ...
   while ($content =~ m{<a.*?href\s*=\s*(['|"]?)\s*([^ '"<>]+)\1.*?>(.+?)</a>}isg) {
      ($urlRef, $link) = ($2, $3);
		$link = Encode::decode('windows-1256', $link);
      $link =~ s/<!.*?-->//g;
      $link =~ s/<.*?>//g;
...
$newspaper   = "Elnashra";
$prefix      = "elnashra";
$defSiteRoot = "http://www.elnashra.com";
...
## newspapers.
##
      # The next line uses information about Elnashra's site organization
   if (($url =~ m{/(articles|news)(-\d+)+.html$}) || ($url =~ m{news.php\?.*?id=\d+})) {
##
## END CUSTOM CODE 2
##
      $title = $links{$url};
