...
public class SiteCrawlerTask extends TimerTask {
    ...
    public SiteCrawlerTask(final String name, final String path, final Date time) {
        ...
        if (_log.isInfoEnabled())
            _log.info("New crawler <" + name + "> with path <" + path + "> scheduled at time <" + time + ">");
    }
                    ...
                    if (cTimes != null)
                        try {
                            ...
                            if (_log.isErrorEnabled())
                                _log.error("Invalid time string " + cTimes + " for crawler " + cName);
                        }
                    else
    ...
    public void run() {
        ...
        if (lastRunTime != null) {
            ...
            if ((now - lrt) < ONE_HOUR) {
                if (_log.isInfoEnabled())
                    _log.info("We last crawled only " + (now - lrt) / (1000 * 60) + " minutes ago!  Skipping this time!");
