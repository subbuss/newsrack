...
##
$newspaper   = "Star of Mysore";
$prefix      = "som";
$defSiteRoot = "http://www.starofmysore.com";
...
## newspapers.
##
      # The next line uses information about Star of Mysore's site organization
		# http://www.starofmysore.com/main.asp?type=news&item=11558
   if (($url =~ m{$defSiteRoot/.*item=\d+}) || ($url =~ m{$defSiteRoot/.*type=editorial})) {
			# For most sites, the next line suffices!
      $title = $links{$url};
		if ($title =~ /^\s*more\s*$/i) {
			$title = &ReadTitle($url, "<font class=\"contentboldNew\">", "</font>");
		}
##
## END CUSTOM CODE 2
##
