...
public class Issue implements java.io.Serializable {
    ...
    private static void genMultiToken_JFLEX_RegExp(PrintWriter pw, String k, HashSet hs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (c) {
                case ' ':
                    re.append(JFLEX_SPACE);
                    break;
                case '[':
                    i = emitPredefinedKeywords(re, i, cs);
                    break;
                default:
                    re.append(c);
                    break;
    ...
    private static void gen_JFLEX_RegExp(String kword, StringBuffer buf) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (c) {
                case ' ':
                    buf.append(Issue.JFLEX_SPACE);
                    break;
                case '[':
                    i = emitPredefinedKeywords(buf, i, cs);
                    break;
                default:
                    buf.append(c);
                    break;
    ...
    private static void gen_JFLEX_RegExps(Concept c, PrintWriter pw, HashMap<String, HashSet> kwToCptMap) {
        ...
        while (it.hasNext()) {
            ...
            HashSet cSet = kwToCptMap.get(kw);
    ...
    public void setFreezeFlag(boolean flag) {
        _frozen = flag;
    }

    public void setPrivateFlag(boolean flag) {
        _private = flag;
    }
    ...
    public void resetMaxNewsID() {
        _db.resetMaxNewsIdForIssue(this);
    }
    ...
    public Collection<Source> getMonitoredSources() {
        return (_sources == null) ? null : _sources;
    }

    public void addSources(Collection<Source> srcs) {
        _sources = srcs;
    }
    ...
    public void setTaxonomyPath(String p) {
        _taxonomyPath = p;
    }
    ...
    public int getNumItemsSinceLastDownload() {
        return _numItemsSinceLastDownload;
    }

    public void setNumItemsSinceLastDownload(int n) {
        _numItemsSinceLastDownload = n;
    }
    ...
    public void setLastUpdateTime(Date d) {
        _lastUpdateTime = d;
    }
...
//			JFlex.Main.main(args);
    ...
    public void classifyArticle(NewsItem ni, int numTokens, HashMap<String, Score> tokTable) {
        ...
        for (Category c : getCategories()) {
            Score s = tokTable.get("[" + c.getName() + "]");
            // The category might have been processed while processing another category
            // Ex: because of a rule like this: [Slums and Courts] = [Slums] and Courts
            // [Slums and Courts] references the [Slums] category and so, while processing
            // this cat, the [Slums] cat might have been processed.
            ...
            if (s == null)
                ...
                _log.debug("CAT " + c.getName() + " in issue " + getName() + " has already been processed!");
    ...
    private void scanAndClassifyNewsItems(Feed f, Collection<NewsItem> newsItems, boolean skipProcessed) {
        if (_log.isDebugEnabled())
            _log.debug("... request to scan and classify for " + getName() + " for feed " + ((f == null) ? null : f.getName()));
        ...
        try {
            ...
            for (NewsItem ni : newsItems) {
                ...
                if (skipProcessed && _db.newsItemHasBeenProcessedForIssue(ni, this)) {
                    if (_log.isDebugEnabled())
                        _log.debug("Ignoring news id " + ni.getKey() + " with title " + ni.getTitle());
                    continue;
                }
        ...
        if (!s.getFeed().isNewsRackFilter()) {
            ...
            while (newsIndexes.hasNext()) {
                ...
                if (_log.isInfoEnabled())
                    _log.info("--> Classifying news for date: " + newsIndex.getCreationTime() + " for feed: " + s.getFeed().getTag());
    ...
    public String getRSSDir() {
        return NewsRack.getBaseRssDir() + _taxonomyPath + File.separator;
    }
