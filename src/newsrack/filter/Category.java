...
// ############### NON-STATIC FIELDS AND METHODS ############
    ...
    public Collection<Category> getChildren() {
        return (_children == null) ? EMPTY_LIST : _children; /* FIXME: any way to return a read-only list?? */
    }
    ...
    public boolean updatedWithinLastNHours(final int numHours) {
        if (_lastUpdateTime == null) {
            if (_log.isErrorEnabled())
                _log.error("NULL _lastUpdateTime for " + _issue.getName() + " and cat " + getName());
            return false;
        }
 ...
 **/
     ...
     */
    public List<NewsItem> getNews(final int n) {
    ...
    public synchronized Score getMatchScore(NewsItem article, int numTokens, Hashtable matchScores) {
        ...
        if (isLeafCategory()) {
            if (_log.isDebugEnabled())
                _log.debug("---- trying to match <" + _name + "> with rule <" + _filter._ruleString + "> ----");
        ...
        if (!matchedCats.isEmpty()) {
            if (_log.isInfoEnabled())
                _log.info("ADDING " + article.getTitle() + " to " + getUser().getUid() + ":" + getIssue().getName() + ":" + _name);
            if (_outputFeed != null)
    ...
    protected void setupForDownloading(Issue issue) {
        if (_log.isDebugEnabled())
            _log.debug("setupFordownloading for issue " + issue.getName() + " and cat " + getName());
