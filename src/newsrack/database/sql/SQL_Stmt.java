...
public enum SQL_Stmt {
    ...
    GET_FILTER_TERMS(
            ...
            new AbstractResultProcessor() {
                public Object processResultSet(ResultSet rs) throws java.sql.SQLException {
                    return new Object[]{rs.getLong(1), rs.getInt(2), rs.getLong(3), rs.getLong(4)};
                }
            },
            false
    ),
    ...
    GET_ISSUE_INFO(
            ...
            new AbstractResultProcessor() {
                public Object processResultSet(ResultSet rs) throws java.sql.SQLException {
                    return new Triple(rs.getLong(1), rs.getInt(2), rs.getTimestamp(3));
                }
            },
            true
    ),
    ...
    UPDATE_FEED_STATS(
            "UPDATE feeds SET num_fetches = ?, num_failures = ? WHERE feed_key = ?",
...
class GetCollectionResultProcessor extends AbstractResultProcessor {
    public ResultProcessor getNewInstance() {
        return new GetCollectionResultProcessor();
    }
...
class GetNewsIndexResultProcessor extends AbstractResultProcessor {
    public Object processResultSet(ResultSet rs) throws java.sql.SQLException {
        return new SQL_NewsIndex(rs.getLong(1), rs.getLong(2), rs.getDate(3));
    }
}
...
class GetPublicFilesResultProcessor extends AbstractResultProcessor {
    public Object processResultSet(ResultSet rs) throws SQLException {
        return new Tuple<String, Long>(rs.getString(1), rs.getLong(2));
    }
...
class GetFilterResultProcessor extends AbstractResultProcessor {
    ...
    public ResultProcessor getNewInstance() {
        return new GetFilterResultProcessor(_getUserKey);
    }
...
class GetCategoryResultProcessor extends AbstractResultProcessor {
    ...
    public ResultProcessor getNewInstance() {
        return new GetCategoryResultProcessor(_getNewsInfo, _buildRuleTree, _buildTaxonomy);
    }
